namespace System.Web.Http.CsHttp
{
    /// <summary>
    ...
    /// Return false to abort parsing.
    /// </summary>
    internal delegate bool OnStartLineParsedCallback(string field1, string field2, string field3);

    /// <summary>
    /// Callback invoked after each header field line is parsed.
    /// Return false to abort parsing.
    /// </summary>
    internal delegate bool OnHeaderParsedCallback(string fieldName, string fieldValue, int headerIndex);

    /// <summary>
    /// Callback invoked when a chunk header is read during chunked decoding.
    /// Return false to abort parsing.
    /// </summary>
    internal delegate bool OnChunkHeaderCallback(long chunkSize, long totalBytesRead);

    /// <summary>
    /// Callback invoked during body reading progress.
    /// Return false to abort parsing.
    /// </summary>
    internal delegate bool OnBodyProgressCallback(int batchSize, long totalBytesRead);
}
